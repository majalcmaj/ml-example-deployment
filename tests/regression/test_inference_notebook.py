...
def inference_run(training_run: None, repo_root: Path, outputs_root: Path) -> None:  # noqa: ARG001 (fixture ordering)
    lib.produce_inference_artifacts(repo_root, outputs_root)


def test_schema_contract_unchanged(outputs_root: Path) -> None:
    actual = lib.load_csv("inference_next_day_forecast.csv", outputs_root=outputs_root)
    expected = lib.load_csv(
        "inference_next_day_forecast.csv", outputs_root=outputs_root, baseline=True
    )
