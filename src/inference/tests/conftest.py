...
NOTEBOOK_PATH = MEMBER_ROOT / "inference" / "daily_product_demand_inference.ipynb"
...
def run_root(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    outputs.mkdir()
    shutil.copy(BASELINE_DIR / "xgb_daily_product_demand.json", outputs / "xgb_daily_product_demand.json")
    shutil.copy(BASELINE_DIR / "forecast_metadata.joblib", outputs / "forecast_metadata.joblib")
