...
def test_is_weekend_flags_saturday_and_sunday() -> None:
    ...
    frame = pd.DataFrame({DATE_COLUMN: dates, CATEGORY_COLUMN: "Coffee", TARGET_COLUMN: 1})

    featured = create_time_features(frame)

    flags = dict(zip(featured[DATE_COLUMN].dt.day_name(), featured[IS_WEEKEND_COLUMN], strict=True))
