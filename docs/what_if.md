...
`docs/deployment.md` is the baseline design. This is a decision record for what changes and
...
follow today — SageMaker expects specific `/opt/ml/{input,output,processing}` conventions, not
the plain `ENTRYPOINT` + config-file/env-var contract this repo already has. Fargate runs the
*exact* image and entrypoint unchanged; a SageMaker move would mean reworking the Dockerfiles
around its I/O conventions for capabilities (distributed training, spot) this single-model,
daily-batch job doesn't use. Revisit if training ever needs multi-node distribution or
spot-driven cost cutting.
...
**Move:** this is the design `docs/deployment.md` deliberately doesn't build today — a DynamoDB
table (predictions keyed by category/date) behind a read Lambda and API Gateway. Inference's write
target changes from the S3 object to a DynamoDB `PutItem`; the rest of inference's compute logic
doesn't change, only the sink.
...
fetch from S3 before startup checks run. This is already supported by the existing contract —
`artifact_dir` is a plain config field, taken as a parameter rather than assuming `/opt/model`
wherever it's used. One env var, zero code change.
...
**Move:** SageMaker Model Registry, or managed MLflow on SageMaker, replacing today's "image tag is
the model version" scheme. A lighter option worth naming before reaching for SageMaker:
[DVC](https://dvc.org) layers an S3-blobs + git-pointer scheme on top of git as an off-the-shelf
tool, instead of hand-rolling that hashing/manifest logic. Considered and declined *for now* — not
an oversight. At this scale (one model, one owner, infrequent retraining) a managed registry, or
even adopting DVC, is setup cost with no immediate payoff; it's the right move once more than one
...
engineering, model load/predict, the artifact contract — is already shaped to be reused here;
only the entrypoint (batch script vs. request handler) and the trigger (EventBridge vs. API
Gateway) change.
...
`docs/deployment.md` for the baseline these branches escalate from, and
`docs/architecture.md` for what's actually built and running today.
